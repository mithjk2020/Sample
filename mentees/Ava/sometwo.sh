...
name=$(whoami)
if [[  $(find /home/admin/mentees -maxdepth 1 -type d -user "$name") ]]; then
   ...
   sudo sh -c "echo \"$name $rollno ${values[0]}=>${values[1]}=>${values[2]}\" >>/home/admin/mentees_domain.txt"
   sudo sh -c "echo '$values[0]}'=>'${values[1]}'=>'$values[2]}' > /home/admin/mentees/'$name'/domain_pref.txt"
   if [ -n "${values[0]}" ]; then
